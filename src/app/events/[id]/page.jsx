...
import { getEventsAllFilter } from "../../../services/olympics";
...
const Page = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await getEventsAllFilter(
          `?discipline=${id}&page=${page}&gender=${gener}`
        );
  ...
  return (
    <div className="min-h-screen w-full flex justify-center items-center flex-wrap flex-col">
      {data.length !== 0 && (
        <>
          <GameTitleEvents title={data[0].discipline_name} />
        </>
      )}
      <div className="flex items-center justify-around bg-primary p-2 w-full">
        ...
        <div className="flex items-center justify-center gap-2 ">
          <label className="form-control w-full max-w-xs">
            <select className="select select-bordered " onChange={(e) => SetGener(e.target.value)}>
